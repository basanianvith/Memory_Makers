// Goa.js — gallery, lightbox, accordion, pricing and validation
(() => {
    'use strict';
    ...
    // initialize main image from first active thumb or existing main background
    const initThumb = document.querySelector('.thumb.active') || document.querySelector('.thumb');
    if (initThumb) setMain(initThumb.dataset.src || initThumb.style.backgroundImage.replace(/url\((['"]?)(.*?)\1\)/, '$2'));
